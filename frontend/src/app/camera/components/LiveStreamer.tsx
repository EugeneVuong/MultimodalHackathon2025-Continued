...
// Ensure the VIDEOSDK auth token is defined
const rawAuthToken = process.env.NEXT_PUBLIC_VIDEOSDK_AUTH_TOKEN;
if (!rawAuthToken) {
  throw new Error("Missing NEXT_PUBLIC_VIDEOSDK_AUTH_TOKEN env var");
}
const authToken: string = rawAuthToken;

// Creating the Stream
type CreateStreamParams = { token: string };

const createStream = async ({ token }: CreateStreamParams): Promise<string> => {
  try {
...
type StreamViewProps = { streamId: string };
function StreamView({ streamId }: StreamViewProps) {
...
// Component prop types
type ParticipantProps = { participantId: string; streamId: string };
function Participant({ participantId, streamId }: ParticipantProps) {
  ...
  // Type of stream returned by useParticipant (includes MediaStreamTrack)
  type ParticipantStream = ReturnType<typeof useParticipant>["webcamStream"];

  // Setup stream for audio/video elements using SDK stream.track
  const setupStream = useCallback(
    (
      ...
      // SDK returns a Stream object with a MediaStreamTrack
      if (!ref.current) return;

      if (stream && shouldPlay) {
        // Extract the MediaStreamTrack and build a real MediaStream
        const track = (stream as { track: MediaStreamTrack }).track;
        const domStream = new MediaStream([track]);
        ref.current.srcObject = domStream;
  ...
  useEffect(() => {
    setupStream(micStream, audioRef, micOn);
  }, [micStream, micOn, setupStream]);
  // Update webcam stream when webcam is on/off
  useEffect(() => {
    setupStream(webcamStream, videoRef, webcamOn);
  }, [webcamStream, webcamOn, setupStream]);
