...
// Assert that env var is present so token prop matches expected string type
...
function Participant({
  ...
  }, [webcamStream, webcamOn, captureSnapshot]);
...
export default function SecurityDashboard() {
  ...
  // Hold the timeout ID in a ref so updating it doesn't trigger re-renders
  const alertTimer = useRef<NodeJS.Timeout | null>(null);

  // Auto-dismiss validation alerts after 5 s
  useEffect(() => {
